...
class NFeService extends DocumentosFiscaisAbstract
{
    public function buildNFeXml(Request $request)
    {
        try {
            ...
            $stdIdeNfe->idDest = $request->input('local_destino');
            ...
            if (!is_null($request->input('itens'))) {
                ...
                foreach ($request->input('itens') as $key => $item) {
                    ...
                    $stdProdutoItem->vFrete = ($request->input("itens.{$key}.valor_frete") === "" || $request->input("itens.{$key}.valor_frete") === 0)
                        ? null
                        : $request->input("itens.{$key}.valor_frete");
                    $stdProdutoItem->vSeg = ($request->input("itens.{$key}.valor_seguro") === "" || $request->input("itens.{$key}.valor_seguro") === 0)
                        ? null
                        : $request->input("itens.{$key}.valor_seguro");
                    $stdProdutoItem->vDesc = ($request->input("itens.{$key}.valor_desconto") === "" || $request->input("itens.{$key}.valor_desconto") === 0)
                        ? null
                        : $request->input("itens.{$key}.valor_desconto");
                    $stdProdutoItem->vOutro = ($request->input("itens.{$key}.valor_outras_despesas") === "" || $request->input("itens.{$key}.valor_outras_despesas") === 0)
                        ? null
                        : $request->input("itens.{$key}.valor_outras_despesas");
            ...
            if($request->input('pessoas_autorizadas')){
    ...
    public function buildDanfeCCe(Documento $documento)
    {
        ...
        try {
            ...
            $danfe = new Daevento(base64_decode($documento->conteudo_xml_autorizado), $dadosEmitente);
            $danfe->debugMode(false);
